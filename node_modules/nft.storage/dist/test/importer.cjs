...
var Multihash = _interopDefault(require('multihashing-async'));
var IPLD = _interopDefault(require('ipld'));
var InMemory = _interopDefault(require('ipld-in-memory'));
var importer = _interopDefault(require('ipfs-unixfs-importer'));
...
class Block {
  ...
  async put(bytes, { cid }) {
    ...
    const node = ipldDagPb.util.deserialize(bytes);
  ...
  async get(cid, options) {
    ...
    if (node instanceof Uint8Array) {
      ...
      return { cid, data: ipldDagPb.util.serialize(node) }
    }
  }
}
