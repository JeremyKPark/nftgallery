...
const test = test => {
  test("test baisc", async () => {
    ...
    test$1.assert.isEqual(typeof lib.File, "function");
  });

  if (globalThis.window === globalThis) {
    test("uses built-ins", async () => {
      test$1.assert.isEqual(lib.File, globalThis.File);
      test$1.assert.isEqual(lib.Blob, globalThis.Blob);
    });
  }
  ...
  test("new File", async () => {
    ...
    test$1.assert.ok(file.lastModified > before);
    test$1.assert.ok(file.lastModified < after);
  ...
  test("File name is (not) escaped", async () => {
    ...
    test$1.assert.ok(file.name === "dir:name" || file.name === "dir/name");
    test$1.assert.equal(file.type, "");
  });
};
