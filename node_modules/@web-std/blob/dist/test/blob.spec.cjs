...
const test = test => {
  test("test baisc", async () => {
    test$1.assert.isEqual(typeof lib.Blob, "function");
    test$1.assert.isEqual(typeof lib.TextDecoder, "function");
    test$1.assert.isEqual(typeof lib.TextEncoder, "function");
    test$1.assert.isEqual(typeof lib.ReadableStream, "function");
  });

  if (globalThis.window === globalThis) {
    test("exports built-ins", () => {
      test$1.assert.equal(lib.Blob, globalThis.Blob);
      test$1.assert.equal(lib.TextDecoder, globalThis.TextDecoder);
      test$1.assert.equal(lib.TextEncoder, globalThis.TextEncoder);
      test$1.assert.equal(lib.ReadableStream, globalThis.ReadableStream);
    });
  }
